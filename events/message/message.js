    ...
    metrics.init({ apiKey: this.client.config.datadogApiKey, host: 'pogy', prefix: 'pogy.' });
      ...
      if (message.content.match(mentionRegex)) {
        ...
        const embed = new MessageEmbed()
          .setTitle('Hello, I\'m ThisIsBot. What\'s Up?')
          .addField(`Prefix`,proofita, true)
          .addField(`Usage`,proofitaa, true)
          .setDescription(`\nIf you like ThisIsBot, Consider [voting](https://discord-botlist.eu/bots/835605460970110996/vote), or [inviting](https://discord.com/oauth2/authorize?client_id=835605460970110996&permissions=8&redirect_uri=https%3A%2F%2Fthisisbot.tk%2Fcallback&scope=bot) it to your server! Thank you for using ThisIsBot, we hope you enjoy it, as we always look forward to improve the bot`)
          .setFooter('Thank you for using ThisIsBot!!')
          .setColor('#FF2C98')
...
message.channel.send(`ThisIsBot is currently undergoing maintenance which wont allow anyone to access ThisIsBots's Commands. Feel free to try again later. For updates: https://discord.gg/Vg3zzuyr98`)
...
return message.channel.send(embed)
  ...
  if (userBlacklistSettings && userBlacklistSettings.isBlacklisted)  return;
      ...
      if (command) {
        ...
        if (typeof rateLimit === "string") return message.channel.send(` ❌ Please wait **${rateLimit}** before running the **${cmd}** command again - ${message.author}\n\n${number === 1 ? "*Did You know that ThisIsBot has its own dashboard? `https://thisisbot.tk/dashboard`*" : ""}${number === 2 ? "*You can check our top.gg page at `https://soon`*" : ""}`).then((s)=>{
