...
module.exports = class extends Command {
    constructor(...args) {
      super(...args, {
        ...
        examples: [ 'report enable #sserver-reports', 'report disable', 'report 25768765276876785 IP logging' ],
        cooldown: 3,
...
if(!channel) return message.channel.send(new MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${fail} ${language.report11}`).setFooter('https://thisisbot.tk').setTimestamp().setColor('RED'));


 var acceptReason = args.splice(1).join(' ');
 if(!acceptReason) return message.channel.send(new MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${fail} ${language.report12}`).setFooter('https://thisisbot.tk').setTimestamp().setColor('RED'));

if(acceptReason.length < 5) return message.channel.send(new MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${fail} ${language.report13}`).setFooter('https://thisisbot.tk').setTimestamp().setColor('RED'));

   if(acceptReason.length > 600 || args.join(' ').length > 600) return message.channel.send(new MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${fail} ${language.report14}`).setFooter('https://thisisbot.tk').setTimestamp().setColor('RED'));
...
if(user.id === message.author.id) return message.channel.send(new MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${fail} ${language.report8}`).setFooter('https://thisisbot.tk').setTimestamp().setColor('RED'));

if(user.bot) return message.channel.send(new MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${fail} ${language.report9}`).setFooter('https://thisisbot.tk').setTimestamp().setColor('RED'));

if(!guildDB.report.reportChannelID || !guildDB.report.reportChannelID === null) return message.channel.send(new MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${fail} ${language.report10}`).setFooter('https://thisisbot.tk').setTimestamp().setColor('RED'));
