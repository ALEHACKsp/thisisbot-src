...
module.exports = class extends Command {
  ...
  async run(message, args) {
    ...
    .setDescription(`❌ | The current does not have any form to apply to`)
    .setColor(message.client.color.red)

        const closed2 = new discord.MessageEmbed()
    .setDescription(`❌ | I could not find the guild's apply Log channel. Please make sure to let an admin know.`)
    .setColor(message.client.color.red)
  ...
  if(!channel) return message.channel.send(closed);
      await message.author.send(new discord.MessageEmbed().setColor(message.client.color.green).setFooter('Powered by thisisbot.tk').setDescription(`✔️ | You can apply to the form in **${message.guild.name}** [by clicking here](https://thisisbot.tk/apply/${message.guild.id})`))
      .then(message.channel.send(`Form sent by DMs - ${message.author}`))
      .catch(()=>{
        return message.channel.send(` ❌ Couldn't send you the form, your dms are closed - ${message.author}`)
        })
