...
module.exports = class extends Command {
  ...
  async run(message, args) {
    ...
    const errorEmbed = new discord.MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png'})).setColor(client.color.red).setFooter("https://thisisbot.tk").setTimestamp()
    const successEmbed = new discord.MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png'})).setColor(client.color.green).setFooter("https://thisisbot.tk").setTimestamp()
    ...
    await inventory.findOne({ user: message.author.id }, async (err, data) => {
      ...
      let iDamage = i.damage
        ...
        if(iBuyable === false) {
          ...
          price = numberWithCommas(iPrice) + " 💸"
        }
        if(iSellable === false) {
          sell = "**Not able to be sold**"
        } else {
          sell = numberWithCommas(iSell) + " 💸"
        }
