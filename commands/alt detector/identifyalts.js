  ...
  constructor(...args) {
   ...
   async run(message, args) {
    ...
    const client = message.client
      ...
      let days = args[0]
      if(!days) return message.channel.send(new discord.MessageEmbed().setColor(client.color.red).setDescription(`❌ | Please provide a valid Days Duration`))

      if(isNaN(days)) return message.channel.send(new discord.MessageEmbed().setColor(client.color.red).setDescription(`❌ | Please provide a valid Days Duration`))
   
    let day = Number(days)

    if(day > 100) return message.channel.send(new discord.MessageEmbed().setColor(client.color.red).setDescription(`❌ | You may only find alts of an account age of **100 days** or below`))
