...
module.exports = class extends Command {
    constructor(...args) {
      super(...args, {
        ...
        category: 'Moderation',
        usage: '<user> [time]',
        examples: [ 'mute @Rico 1h Stop spamming' ],
        guildOnly: true,
  ...
  const language = require(`../../data/language/${guildDB.language}.json`)
    ...
    let muteRole = await message.guild.roles.cache.get(logging.moderation.mute_role);
     ...
     if (!mentionedMember) {
        ...
        return message.channel.send(new Discord.MessageEmbed()
            .setDescription(`❌ | ${language.muteTime}`)
            .setColor(client.color.red))
    }
    ...
    if (mentionedMember.roles.highest.position >= message.guild.me.roles.highest.position) {
        return message.channel.send(new Discord.MessageEmbed()
            .setDescription(`❌ | ${language.muteRolePosition}`)
            .setColor(client.color.red))
    }
    else if (muteRole.position >= message.guild.me.roles.highest.position) {
        return message.channel.send(new Discord.MessageEmbed()
            .setDescription(`❌ | ${language.muteRolePositionBot}`)
            .setColor(client.color.red))
    }
    ...
    if (isMuted) {
        return message.channel.send(new Discord.MessageEmbed()
            .setDescription(`❌ | ${language.muteMuted}`)
            .setColor(client.color.red))
    }
...
dmEmbed = `❌ You've been muted in **${message.guild.name}** for **${msRegex.exec(args[1])[1]}**`
...
.setDescription(dmEmbed)
    ...
    message.channel.send(new Discord.MessageEmbed().setColor(message.client.color.green).setDescription(`✔️ | Muted **${mentionedMember.user.tag}** for **${msRegex.exec(args[1])[1]}** ${logging && logging.moderation.include_reason === "true" ?`\n\n**Reason:** ${reason}`:``}`)).then(async(s)=>{
