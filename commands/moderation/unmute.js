...
const muteRole = message.guild.roles.cache.find(r => r.name == 'Muted')
 ...
 if (!mentionedMember) {
  return message.channel.send(new Discord.MessageEmbed()
      .setDescription(`❌ | ${language.unmuteNoUser}`)
      .setColor(client.color.red))
}
else if (!muteRole) {
  return message.channel.send(new Discord.MessageEmbed()
      .setDescription(`❌ | ${language.unmuteNoMutedRole}`)
      .setColor(client.color.red))
}
...
const reason = args.slice(1).join(' ') || language.unbanNoReason
    ...
    message.channel.send(new Discord.MessageEmbed().setColor(message.client.color.green).setDescription(`✔️ | Unmuted **${mentionedMember.user.tag}** ${logging && logging.moderation.include_reason === "true" ?`\n\n**Reason:** ${reason}`:``}`)).then(async(s)=>{
