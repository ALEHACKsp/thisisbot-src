...
const language = require(`../../data/language/${guildDB.language}.json`)
    ...
    .setTitle(`❌ Slow Mode Error`)
    .setDescription(`I cant view the provided channel`)
    .setTimestamp()
    ...
    if (rate === '0') {
      message.channel.send(new MessageEmbed()
        .setDescription(`✔️ Slow Mode was successfuly disabled${logging && logging.moderation.include_reason === "true" ?`\n\n**Reason:** ${reason}`:``}`)
        .setColor(message.guild.me.displayHexColor)
      ).then(async(s)=>{
      ...
      message.channel.send(new MessageEmbed()
        .setDescription(`✔️ | Slow Mode was successfuly enabled to **1 msg /${rate}s** ${logging && logging.moderation.include_reason === "true" ?`\n\n**Reason:** ${reason}`:``}`)
        .setColor(message.guild.me.displayHexColor)
      ).then(async(s)=>{
          if(logging && logging.moderation.delete_reply === "true"){
            setTimeout(()=>{
