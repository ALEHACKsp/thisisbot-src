...
return message.channel.send(new discord.MessageEmbed()
   .setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ dynamic: true }))
    .setDescription(`❌ | ${language.rmPosition}`)
    .setTimestamp(message.createdAt)
    .setColor(client.color.red))
}
...
return message.channel.send(new discord.MessageEmbed()
      .setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ dynamic: true }))
    .setDescription(`❌ | ${language.rmNoWarning}`)
    .setTimestamp(message.createdAt)
    .setColor(client.color.red))
}
...
if(!warningID) return message.channel.send(new discord.MessageEmbed()
 .setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ dynamic: true }))
    .setDescription(`❌ | ${language.rmWarnInvalid} `)
    .setTimestamp(message.createdAt)
    .setColor(client.color.red))
...
.setDescription(`✔️ | Cleared Warn **#${warningID}** from **${mentionedMember.user.tag}** ${logging && logging.moderation.include_reason === "true" ?`\n\n**Reason:** ${reason}`:``}`)
