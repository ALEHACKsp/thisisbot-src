...
module.exports = class extends Command {
    constructor(...args) {
      super(...args, {
        ...
        examples: ['purge 20', 'purge #general 10', 'purge @rico 50', 'purge #general @rico 5'],
        guildOnly: true,
...
const language = require(`../../data/language/${guildDB.language}.json`)
    ...
    if (isNaN(amount) === true || !amount || amount < 0 || amount > 100)
      ...
      .setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ dynamic: true }))
      .setTitle(`❌ Clear Error`)
      .setDescription(`I can only purge 1 - 100 messages`)
      .setTimestamp()
      .setFooter('https://thisisbot.tk')
      .setColor(message.guild.me.displayHexColor));
    ...
    if (!channel.permissionsFor(message.guild.me).has(['MANAGE_MESSAGES']))
      ...
      .setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ dynamic: true }))
      .setTitle(`❌ Clear Error`)
      .setDescription(`Please make sure i have the **Manage Messages** Permissions`)
      .setTimestamp()
      .setFooter('https://thisisbot.tk')
      .setColor(message.guild.me.displayHexColor));
    ...
    if (messages.size === 0) { 
      ...
      channel.bulkDelete(messages, true).then(messages => {
        const embed = new MessageEmbed()
           ...
           .setDescription(`
            ✔️ Successfully deleted **${messages.size}** message(s) ${logging && logging.moderation.include_reason === "true" ?`\n\n`:``}
            `)
          .setColor(message.guild.me.displayHexColor);
