module.exports = async function (channel, message, options) {
  const init = new Promise(async resolve => { // Create Promise
    ...
    // Assign Webhook
    let hook = webhooks.find(w => w.name === 'https://thisisbot.tk')
    if (!hook) {
        try {
            hook = await channel.createWebhook('https://thisisbot.tk', {
                avatar: `https://thisisbot.tk/logo.png`
            });
        } catch (e) {
            hook = await channel.createWebhook('https://thisisbot.tk', `https://thisisbot.tk/logo.png`);
        }
