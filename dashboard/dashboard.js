...
const secret = config.secret;
  ...
  module.exports = async (client) => {
    ...
    passport.use(new Strategy({
      clientID: `${clientID}`,
      clientSecret: `${secret}`,
      callbackURL: `https://thisisbot.tk/callback`,
  ...
  store :MongoStore.create({ mongoUrl: jsonconfig.mongodb_url})
    ...
    const renderTemplate = (res, req, template, data = {}) => {
      ...
      const baseData = {
        ...
        url: res,
        req: req,
        image: `https://thisisbot.tk/logo.png`,
...
var url = req.protocol + '://' + req.get('host') + req.originalUrl;
    ...
    app.get("/support", (req, res) => {
      res.redirect(`https://discord.gg/Vg3zzuyr98`)
    });
...
res.redirect(`https://discord.com/oauth2/authorize?client_id=835605460970110996&permissions=8&redirect_uri=https%3A%2F%2Fthisisbot.tk%2Fcallback&scope=bot`)
});
...
member.send(new MessageEmbed().setColor('GREEN').setFooter(`Powered by https://thisisbot.tk`).setTitle(`Application #${ticketID}`).setDescription(`Hey ${member.user.username}! Your form was Submitted and ready to be judged.\n\n**Form ID**: \`${ticketID}\`\n**Time:** ${moment(new Date()).format("dddd, MMMM Do YYYY HH:mm:ss")}`)).catch(()=>{});
...
channel.send(embed)
 ...
 app.get("/redeem", checkAuth, (req, res) => {
    ...
    app.get("/dashboard", checkAuth, (req, res) => {
      ...
      const server = client.guilds.cache.get('840615663285305374');
...
if(results.length >= 10) return;
    ...
    app.post("/report", async (req, res) => {
      ...
      if (req.body.type === "report") {
        const reportEmbed = new WebhookClient('842465784743854080', 'ZxrSEZ3GhOvTwVLi69X6s5r2nu1YzAEy-ZqVwsbjMypzW3b4nyZ8nqJAxnfLQHInqqWN');

        const report = new MessageEmbed()
          .setColor('GREEN')
          .setTitle(`ThisIsBot Reports`)
      ...
      if (Object.prototype.hasOwnProperty.call(data, "sendEmbed")) {
        ...
        if (ticketSettings.ticketToggle == "true") {
          ...
          if (ticketSettings.ticketType == "reaction" && ticketSettings.ticketCustom == "false") {
            ...
            if (storedSettings.isPremium == "false") {
              ticketSettings.ticketFooter = "Powered by thisisbot.tk";
            } else {
            ...
            let reactionFooter = "Powered by thisisbot.tk";



            let footer = "Powered by thisisbot.tk";
            ...
            if (storedSettings.isPremium == "false") {
              ticketEmbed.setFooter(`Powered by thisisbot.tk`)
            } else {
...
if(maintenance && maintenance.toggle == "true") {
    ...
    app.post('/dblwebhook', webhook.middleware(), async (req) => {
      ...
      const msg = new Discord.MessageEmbed()
        ...
        .setDescription(`Thank you **${apiUser.username}#${apiUser.discriminator}** (${apiUser.id}) for voting **ThisIsBot**!`)
      Hook.send(msg);
